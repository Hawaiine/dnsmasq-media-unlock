...
> - 🎬 **匹配解锁规则** → 走解锁 DNS
> - 🌐 **其他流量** → 走普通 DNS（默认 223.5.5.5 / 119.29.29.29）
>
> 标准的 split-DNS 方案。锁定方法见上方第 4️⃣ 步。

### 🤔 为什么上游 DNS 不用 1.1.1.1 / 8.8.8.8？

> 默认上游已改为更适合国内网络的 **阿里 DNS（223.5.5.5）** 和 **腾讯 DNS（119.29.29.29）**，延迟低、稳定。你也可以自行修改：
> ```bash
> sed -i 's/223.5.5.5/你偏好的DNS/g' etc/dnsmasq.conf
> ```
